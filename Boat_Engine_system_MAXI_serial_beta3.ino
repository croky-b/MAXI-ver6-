...
SerialTransfer audioTransfer;

struct STRUCT{  
  volatile boolean engineOn= false;
  volatile boolean fishing= false;
  volatile boolean fog= false;
  volatile boolean manoeuvred= false;
  volatile boolean manoeuvred_L= false;
  volatile boolean manoeuvred_R= false;
  volatile boolean manoeuvred_Rev= false;
  volatile boolean caution= false;
  volatile boolean caution_L= false;
  volatile boolean caution_R= false;
  volatile boolean caution_OK= false;
  volatile boolean anchoring= false; 
  volatile boolean gun= false; 
  volatile boolean AAgun= false;
  volatile boolean bell= false;
  volatile boolean shortHorn= false;
  volatile boolean longHorn= false;
  volatile boolean seagull= false; 
  volatile boolean ring= false;
  volatile boolean ambient= false;
  volatile boolean alarm= false;
  volatile boolean ancor= false;
  volatile boolean eleven1= false;
  volatile boolean eleven2= false;
  volatile boolean eleven3= false;
  volatile boolean eleven4= false;
  volatile boolean eleven5= false;
  volatile boolean eleven6= false;
  volatile boolean eleven7= false;
  volatile boolean eleven8= false;
  volatile boolean twelve1= false;
  volatile boolean twelve2= false;
  volatile boolean twelve3= false;

volatile boolean act11A_L_On2 = false;
volatile boolean act11A_H_On2= false;
volatile boolean act11B_L_On2 = false;
volatile boolean act11B_H_On2 = false;
volatile boolean act11C_L_On2 = false;
volatile boolean act11C_H_On2 = false;
volatile boolean act11D_L_On2 = false;
volatile boolean act11D_H_On2 = false;

  
  uint8_t act11A_L_Rep =0;
  uint8_t act11A_H_Rep =0;
  uint8_t act11B_L_Rep= 0;
  uint8_t act11B_H_Rep= 0;
  uint8_t act11C_L_Rep =0;
  uint8_t act11C_H_Rep=0;
  uint8_t act11D_L_Rep=0;
  uint8_t act11D_H_Rep=0;
  uint8_t type=0;
  uint8_t size=0;
...
bool old_fog;
bool old_manoeuvre;
bool old_manoeuvre_L;
bool old_manoeuvre_R;
bool old_manoeuvre_Rev;
bool old_avertissement;
bool old_avertissement_L;
bool old_avertissement_R;
bool old_avertissement_OK;
bool old_mouillage;
bool old_gun ;
bool old_AAgun;
bool old_bell;
...
volatile boolean manoeuvre_L;
volatile boolean manoeuvre_R;
volatile boolean manoeuvre_Rev;

volatile boolean avertissement_L;
volatile boolean avertissement_R;
volatile boolean avertissement_OK;

volatile boolean gun_teensy;
volatile boolean AAgun_teensy;
volatile boolean bell_teensy;
volatile boolean short_teensy;
volatile boolean long_teensy;
volatile boolean seagull_teensy;
volatile boolean ring_teensy;
volatile boolean ambient_teensy;
volatile boolean alarm_teensy;
volatile boolean ancor_teensy;
volatile boolean eleven1_teensy;
volatile boolean eleven2_teensy;
volatile boolean eleven3_teensy;
volatile boolean eleven4_teensy;
volatile boolean eleven5_teensy;
volatile boolean eleven6_teensy;
volatile boolean eleven7_teensy;
volatile boolean eleven8_teensy;
volatile boolean twelve1_teensy;
volatile boolean twelve2_teensy;
volatile boolean twelve3_teensy;

/*
uint16_t eleven1_Repeat_teensy;
uint16_t eleven2_Repeat_teensy;
uint16_t eleven3_Repeat_teensy;
uint16_t eleven4_Repeat_teensy;
uint16_t eleven5_Repeat_teensy;
uint16_t eleven6_Repeat_teensy;
uint16_t eleven7_Repeat_teensy;
uint16_t Seagull_Repeat_teensy;
uint16_t ambient_Repeat_teensy;
*/
volatile boolean act11A_L_On = false;
volatile boolean act11A_H_On = false;
volatile boolean act11B_L_On = false;
volatile boolean act11B_H_On = false;
volatile boolean act11C_L_On = false;
volatile boolean act11C_H_On = false;
volatile boolean act11D_L_On = false;
volatile boolean act11D_H_On = false;
...
PWMTrigger CH16;
PWMTrigger CHAct4;
PWMTrigger CHAct5turn;
PWMTrigger CHAct5confirm;
...
volatile boolean Manoeuvre = false;
...
volatile boolean Fishing_Sound = false;
...
void setup()
{
 ...
 // Main SX1509
  ...
  Serial.begin(115200);
  Serial1.begin(115200,SERIAL_8N1,40,2);
  audioTransfer.begin(Serial1);
...
{ 
  ...
  CHAct4.update(pulseWidth[Act4CH - 1]);
  #endif
  ...
  Action5.update(pulseWidth[ACTION5CH-1]); 
    #endif
  CHAct5turn.update(pulseWidth[Act5turnCH - 1]);
  CHAct5confirm.update(pulseWidth[Act5confirmCH - 1]);
  #endif
...
void engineMassSimulation()
{
 ...
 Serial.print("currentRpm :");
  ...
  Serial.print("alarm_teensy :");
  Serial.println(alarm_teensy);

  Serial.print("ring_teensy :");
  Serial.println(ring_teensy);
...
void engineOnOff()
{
  ...
  if (engineManualOnOff)   // Engine manually switched on or off depending on CH2
  {
    ...
    if (pulseWidth[chManualOnOff - 1] > (1500 + 180) && pulseWidth[chManualOnOff - 1] < 2100)
    {
    ...
    else if (pulseWidth[chManualOnOff - 1] < 1680)
    {
      if (! Avertissement && !manualServo){
...
//#if defined TEENSY_AUDIO

audioData.current_Rpm = currentRpm;

audioData.engineOn = engineOn;
audioData.fishing = Fishing;
audioData.fog = Fog;
audioData.manoeuvred =  Manoeuvre;
audioData.manoeuvred_L = manoeuvre_L;
audioData.manoeuvred_R = manoeuvre_R;
audioData.manoeuvred_Rev = manoeuvre_Rev;
audioData.caution = Avertissement ;
audioData.caution_L =avertissement_L;
audioData.caution_R = avertissement_R;
audioData.caution_OK = avertissement_OK;
audioData.anchoring = Mouillage ;
audioData.gun = gun_teensy;
audioData.AAgun = AAgun_teensy;
audioData.bell = bell_teensy;
audioData.shortHorn = short_teensy;
audioData.longHorn = long_teensy;
audioData.seagull = seagull_teensy;
audioData.ring = ring_teensy;
audioData.ambient= ambient_teensy;
audioData.alarm = alarm_teensy;
audioData.ancor = ancor_teensy;
audioData.eleven1 = eleven1_teensy;
audioData.eleven2 = eleven2_teensy;
audioData.eleven3 = eleven3_teensy;
audioData.eleven4 = eleven4_teensy;
audioData.eleven5 = eleven5_teensy;
audioData.eleven6 = eleven6_teensy;
audioData.eleven7 = eleven7_teensy;
audioData.eleven8 = eleven8_teensy;
audioData.twelve1 = twelve1_teensy;
audioData.twelve2 = twelve2_teensy;
audioData.twelve3 = twelve3_teensy;



audioData.act11A_L_On2 = act11A_L_On;
audioData.act11A_H_On2 = act11A_H_On;
audioData.act11B_L_On2 = act11B_L_On;
audioData.act11B_H_On2 = act11B_H_On;
audioData.act11C_L_On2 = act11C_L_On;
audioData.act11C_H_On2 = act11C_H_On;
audioData.act11D_L_On2 = act11D_L_On;
audioData.act11D_H_On2 = act11D_H_On;


#ifdef ACTION11A_LOW_REPEAT
audioData.act11A_L_Rep = ACTION11A_LOW_REPEAT;
#endif

#ifdef ACTION11A_HIGH_REPEAT
audioData.act11A_H_Rep = ACTION11A_HIGH_REPEAT;
#endif

#ifdef ACTION11B_LOW_REPEAT
audioData.act11B_L_Rep = ACTION11B_LOW_REPEAT;
#endif

#ifdef ACTION11B_HIGH_REPEAT
audioData.act11B_H_Rep = ACTION11B_HIGH_REPEAT;
#endif

#ifdef ACTION11C_LOW_REPEAT
audioData.act11C_L_Rep = ACTION11C_LOW_REPEAT;
#endif

#ifdef ACTION11C_HIGH_REPEAT
audioData.act11C_H_Rep = ACTION11C_HIGH_REPEAT;
#endif

#ifdef ACTION11D_LOW_REPEAT
audioData.act11D_L_Rep = ACTION11D_LOW_REPEAT;
#endif

#ifdef ACTION11D_HIGH_REPEAT
audioData.act11D_H_Rep = ACTION11D_HIGH_REPEAT;
#endif
...
audioData.size = 1;
 ...
 uint16_t sendSize = 0;
  ...
  sendSize = audioTransfer.txObj(audioData, sendSize);
  audioTransfer.sendData(sendSize);
